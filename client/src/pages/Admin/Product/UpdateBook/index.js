  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      const { bookId, name, author, genre, publisher, description, 
        year, pages, size, price, discount, image } = formik.values;      try {
        if (image) {
          // Sử dụng API upload của server thay vì gọi trực tiếp Cloudinary
          const uploadApi = (await import('../../../../api/uploadApi')).default;
          const resUpload = await uploadApi.uploadImage(image);
          const { secure_url, public_id } = resUpload.data;
